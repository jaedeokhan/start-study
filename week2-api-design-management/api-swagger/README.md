...
| 테이블 | 주요 컬럼 | 동시성 제어 | 용도 |
|--------|----------|-----------|------|
| users | id, name, balance | FOR UPDATE | 사용자 및 잔액 관리 |
| products | id, name, price, stock | FOR UPDATE | 상품 및 재고 관리 |
...
| coupon_events | name, total_quantity, issued_quantity | FOR UPDATE | 쿠폰 이벤트 |
...
### Q2: 비관적 락을 사용하는 이유는?
**A**: 재고, 쿠폰 수량, 잔액 등 정합성이 중요한 데이터에 대해 동시성 제어를 통해 데이터 무결성을 보장하기 위함입니다.
