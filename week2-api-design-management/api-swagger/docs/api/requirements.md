...
#### 2.1.1 상품 정보 조회
- **FR-PROD-001**: 시스템은 상품의 기본 정보(상품명, 설명 등)를 조회할 수 있어야 한다.
- **FR-PROD-002**: 시스템은 상품의 가격 정보를 조회할 수 있어야 한다.
- **FR-PROD-003**: 시스템은 상품의 현재 재고 수량을 실시간으로 조회할 수 있어야 한다.
- **FR-PROD-004**: 시스템은 여러 상품 목록을 조회할 수 있어야 한다.
- **FR-PROD-005**: 시스템은 특정 상품의 상세 정보를 조회할 수 있어야 한다.
...
#### 2.2.3 결제 처리
- **FR-PAY-001**: 시스템은 사용자의 잔액 정보를 관리해야 한다.
- **FR-PAY-002**: 사용자는 본인의 잔액을 조회할 수 있어야 한다.
- **FR-PAY-003**: 사용자는 잔액을 충전할 수 있어야 한다.
- **FR-PAY-004**: 시스템은 주문 금액과 사용자 잔액을 비교하여 결제 가능 여부를 확인해야 한다.
- **FR-PAY-005**: 시스템은 결제 시 쿠폰 할인을 적용할 수 있어야 한다.
- **FR-PAY-006**: 시스템은 결제 완료 시 사용자 잔액을 차감해야 한다.
- **FR-PAY-007**: 시스템은 잔액이 부족한 경우 결제를 거부해야 한다.
...
- **NFR-INTG-003**: 시스템은 동시에 잔액 차감이 발생해도 사용자 잔액의 정합성을 보장해야 한다.
...
- **NFR-SEC-001**: 시스템은 사용자의 잔액 정보를 안전하게 관리해야 한다.
