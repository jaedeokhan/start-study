...
- [ ] synchronized 또는 ReentrantLock을 통해 동시성을 제어한다
- [ ] 재고는 절대 음수가 되지 않는다
...
### US-PAY-002: 포인트 충전
**As a** 고객
**I want** 내 계정에 포인트를 충전하고 싶다
**So that** 상품을 구매할 수 있다

**Acceptance Criteria:**
- [ ] 충전할 금액을 입력하여 포인트를 충전할 수 있다
- [ ] 충전 금액은 0보다 커야 한다
- [ ] 충전 후 즉시 포인트에 반영된다
- [ ] 충전 이력이 point_histories 테이블에 기록된다
- [ ] 동시에 여러 충전이 발생해도 포인트가 정확하게 증가한다

**Related Requirements:** FR-PAY-003, FR-PAY-008, NFR-INTG-003

---

### US-PAY-003: 주문 결제 (쿠폰 미적용)
**As a** 고객
**I want** 생성한 주문을 포인트로 결제하고 싶다
**So that** 상품을 구매 완료할 수 있다

**Acceptance Criteria:**
- [ ] 주문 금액과 사용자 포인트를 비교하여 결제 가능 여부를 확인한다
- [ ] 포인트가 충분한 경우 결제가 완료된다
- [ ] 결제 완료 시 주문 상태가 "완료"로 변경된다
- [ ] 결제 완료 시 주문한 상품의 재고가 차감된다
- [ ] 결제 완료 시 사용자 포인트가 차감된다
- [ ] 결제 이력이 point_histories 테이블에 기록된다
- [ ] 포인트가 부족한 경우 결제가 실패하고 에러 메시지를 반환한다
- [ ] 결제 실패 시 재고는 차감되지 않는다
- [ ] 결제-재고차감-포인트차감은 트랜잭션으로 처리된다 (원자성 보장)

**Related Requirements:** FR-PAY-004, FR-PAY-006, FR-PAY-007, FR-PAY-008, FR-ORDR-004, NFR-INTG-004, NFR-INTG-005
...
**As a** 고객
**I want** 내 포인트 사용/충전 이력을 조회하고 싶다
**So that** 포인트 변동 내역을 확인할 수 있다

**Acceptance Criteria:**
- [ ] 사용자의 모든 포인트 이력을 조회할 수 있다
- [ ] 각 이력에는 거래 유형(충전/사용/환불), 금액, 거래 후 잔액이 표시된다
- [ ] 이력은 최신순으로 정렬된다
- [ ] 주문 결제와 연결된 이력은 주문 ID가 함께 표시된다
- [ ] 이력이 없는 경우 빈 목록을 반환한다

**Related Requirements:** FR-PAY-008
